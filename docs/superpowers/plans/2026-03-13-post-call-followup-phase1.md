# Post-Call Follow-Up (Phase 1) — Implementation Plan

> **For agentic workers:** REQUIRED: Use superpowers:subagent-driven-development (if subagents available) or superpowers:executing-plans to implement this plan. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Build a skill that takes a Gong call summary, identifies product questions, researches answers from the Gem help center, and delivers a formatted follow-up message via Gmail draft (external) or Slack (internal).

**Architecture:** A single skill file (`SKILL.md`) that orchestrates a 5-step flow: classify product questions from the Gong extractor output → human gate for confirmation and delivery choice → research answers via WebFetch against help.gem.com → draft the message in the appropriate format → deliver via Gmail MCP (external draft) or Slack MCP (internal send). The skill pauses at the human gate and waits for user input before proceeding.

**Tech Stack:** WebFetch (help.gem.com search), Google Calendar MCP (`gcal_list_events`), Gmail MCP (`gmail_create_draft`), Slack MCP (`slack_send_message`), Markdown skill file with YAML frontmatter

**Spec:** `docs/superpowers/specs/2026-03-13-post-call-followup-phase1-design.md`

---

## Chunk 1: Skill Scaffolding + Question Classification (Steps A-B)

### Task 1: Read reference files

**Files:**
- Read: `skills/gong-call-summary-extractor/SKILL.md` — upstream skill, input format
- Read: `skills/prep-note-summary/SKILL.md` — pattern reference for complex multi-step skills
- Read: `docs/superpowers/specs/2026-03-13-post-call-followup-phase1-design.md` — the design spec

- [ ] **Step 1: Read all three reference files**

Understand the YAML frontmatter conventions, skill structure patterns, the upstream skill's output format (which becomes this skill's input), and every detail in the design spec.

---

### Task 2: Create the SKILL.md with frontmatter, overview, and Steps A-B

**Files:**
- Create: `skills/post-call-followup/SKILL.md`

- [ ] **Step 2: Create the skill file with frontmatter through Step B**

Create `skills/post-call-followup/SKILL.md`. Copy everything between the `<!-- SKILL.MD START -->` and `<!-- SKILL.MD END -->` markers below into the file:
...
```bash
git add skills/post-call-followup/SKILL.md
git commit -m "feat: scaffold post-call-followup skill with question classification and human gate (Steps A-B)"
```

---

## Chunk 2: Product Knowledge Research (Step C)

### Task 3: Add Step C — help center research

**Files:**
- Modify: `skills/post-call-followup/SKILL.md` — append Step C after the Step B section

- [ ] **Step 4: Append Step C to the SKILL.md**

Add the following content immediately after the Step B section (after the line "After this gate, you have: a confirmed question list + delivery target (internal or external)."):
...
- [ ] **Step 5: Commit Step C**

```bash
git add skills/post-call-followup/SKILL.md
git commit -m "feat: add product knowledge research step (Step C) to post-call-followup"
```

---

## Chunk 3: Message Drafting + Delivery (Steps D-E)

### Task 4: Add Step D — message drafting

**Files:**
- Modify: `skills/post-call-followup/SKILL.md` — append Step D after Step C

- [ ] **Step 6: Append Step D to the SKILL.md**

Add the following content immediately after the Step C section:
...
- [ ] **Step 7: Append Step E — delivery**

Add the following content immediately after Step D:
...
- [ ] **Step 8: Commit Steps D-E**

```bash
git add skills/post-call-followup/SKILL.md
git commit -m "feat: add message drafting and delivery steps (Steps D-E) to post-call-followup"
```

---

## Chunk 4: Guidelines, Error Handling, and Phase Notes

### Task 5: Add closing sections

**Files:**
- Modify: `skills/post-call-followup/SKILL.md` — append guidelines, error handling, and phase notes after Step E

- [ ] **Step 9: Append guidelines and error handling**

Add the following content at the end of the SKILL.md, after the Step E section:
...
- [ ] **Step 10: Commit the closing sections**

```bash
git add skills/post-call-followup/SKILL.md
git commit -m "feat: add guidelines, error handling, and phase notes to post-call-followup"
```

---

## Chunk 5: Live Verification

### Task 6: End-to-end verification with real data

- [ ] **Step 11: Run the gong-call-summary-extractor to get a real call summary**

Invoke the `gong-call-summary-extractor` skill to pull a recent Gong call summary. This produces the input for the post-call-followup skill.

```
Pull my latest Gong call summary
```

- [ ] **Step 12: Run the post-call-followup skill with the real summary**

Invoke the `post-call-followup` skill using the output from Step 11.

```
Run post-call follow-up on that call summary
```

Verify each step works correctly:
- **Step A:** Does it correctly identify product questions from the key points and next steps? Are logistics and admin items correctly excluded?
- **Step B:** Does it pause and present the questions for review? Does it wait for user input?
- **Step C:** Does WebFetch successfully search help.gem.com? Are answers extracted from relevant articles?
- **Step D:** Is the message formatted correctly for the chosen delivery target?
- **Step E:** Does delivery succeed (Gmail draft or Slack message)?

- [ ] **Step 13: Fix any issues found during verification**

If any step doesn't work as expected, edit `skills/post-call-followup/SKILL.md` to fix the issue. Common things to watch for:
- WebFetch URL encoding issues with help.gem.com search
- Calendar invite lookup not matching by the right fields
- Message formatting issues (missing newlines, wrong greeting)
- Participant name matching failures

- [ ] **Step 14: Final commit after verification**

```bash
git add skills/post-call-followup/SKILL.md
git commit -m "fix: adjustments from live verification of post-call-followup skill"
```

Only create this commit if changes were made in Step 13. If verification passed cleanly, skip this step.
