...
Short bulleted list of anything in the notes that could derail the deal or complicate the demo. These are signals the SC should walk in aware of — competitive threats, internal politics, past failed evaluations, misaligned stakeholders, budget constraints, or timeline pressure. Only include this section if the notes contain real signals; don't manufacture risks.

Example format:
- **Ashby's ATS appeal** — Derek finds Ashby's ATS compelling, which could pull the deal if we don't reinforce the value of keeping Greenhouse + adding Gem
- **Integration sensitivity** — A previous Lever deal died over integration issues; need to proactively demo Workday integration

## Discovery Questions
2-4 targeted questions derived from gaps or ambiguities in the prep notes. Each question should include a short parenthetical rationale explaining what the rep is trying to learn, so they can prioritize on the fly during the conversation.
...
When significant gaps exist, add a final section to the plan:

```markdown
## Info to Gather Before the Call
A short checklist of the most important missing pieces the rep should try to track down before the demo. Focus on gaps that would meaningfully change how the demo is run — don't just list every omitted section.

Example format:
- [ ] Recruiting team size and structure (affects which workflows to demo)
- [ ] Current ATS and sourcing tools (determines integration story)
- [ ] Other tools being evaluated (need for competitive positioning)
```

Only include this section when the notes have real gaps that would affect demo prep. If the plan is mostly complete, skip it.
